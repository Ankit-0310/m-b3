...
stopuser = {}
token = '7250306654:AAGG_TCo5b0xJcKhBFS3a9KBdbSFZiUOpbo'
	...
	def my_function():
	  ...
	  "timer": "none",
		...
		if BL == '𝗙𝗥𝗘𝗘':	
			...
			contact_button = types.InlineKeyboardButton(text="✨ 𝗢𝗪𝗡𝗘𝗥  ✨", url="https://t.me/Roronoa_robot")
...
𝒀𝑶𝑼𝑹 𝑷𝑳𝑨𝑵 𝑵𝑶𝑾 {BL}</b>
	''',reply_markup=keyboard)
			return
		keyboard = types.InlineKeyboardMarkup()
		contact_button = types.InlineKeyboardButton(text="✨ 𝗝𝗢𝗜𝗡 ✨", url="https://t.me/Err0rrx")
		keyboard.add(contact_button)
...
𝘽𝙤𝙩 𝘽𝙮 @Roronoa_robot''', reply_markup=mes)
...
def start(message):
	def my_function():
		...
		try:
			...
			pas ='ZORO-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))+'-'+''.join(random.choices(characters, k=4))
			current_time = datetime.now()
